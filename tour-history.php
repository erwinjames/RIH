...
if (isset($_REQUEST['payId'])) {
	$pid=intval($_GET['payId']);
	$Paystatus="check";
$sql = "UPDATE tblbooking SET payment=:pstatus WHERE UserEmail=:email and BookingId=:pid";
$query = $dbh->prepare($sql);
$query -> bindParam(':pstatus',$Paystatus, PDO::PARAM_STR);
$query-> bindParam(':email',$email, PDO::PARAM_STR);
...
$sql = "SELECT tblbooking.BookingId as bookid,tblbooking.PackageId 
as pkgid,tbltourpackages.PackageName as packagename,tblbooking.FromDate 
as fromdate,tblbooking.status as status,tblbooking.RegDate 
as regdate,tblbooking.pkgOffer_value as packageOffer,tblbooking.pax as paxPerValue,tblbooking.CancelledBy as cancelby,tblbooking.UpdationDate 
as upddate from tblbooking join tbltourpackages on tbltourpackages.PackageId=tblbooking.PackageId
...
<td>#RIHBoat<?php echo htmlentities($result->bookid);?></td>
...
if(($result->packageOffer==true)){
echo htmlentities($result->packageOffer);
}
?>
</td>
<td>
	<?php
	echo $result->paxPerValue;
